...
@pytest.mark.asyncio
async def test_chat_global_cold_start_returns_envelope(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert isinstance((body.get("reply") or {}).get("hits"), list)
